...
def extract_employee_id(emp):
    w = emp.get("workerID")
    if isinstance(w, dict): return w.get("idValue","")
    return w or ""
...
def get_status(emp):
    ...
    if not hd: return "Inactive"
    h = datetime.fromisoformat(hd)
    if h.tzinfo is None: h=h.replace(tzinfo=timezone.utc)
    now = datetime.now(timezone.utc)
    if h>now: return "Inactive"
    if not td: return "Active"
...
def provision_user_in_ad(user_data, conn, ldap_search_base, ldap_create_base):
    ...
    sam = sanitize_string_for_sam((first[0].lower() + last.lower()) if first and last else "")
    email = (
    ...
    mandatory = {
        ...
        "userPrincipalName", "mail", "sAMAccountName",
...
def debug_employee(req: func.HttpRequest) -> func.HttpResponse:
    token = get_adp_token()
    if not token: return func.HttpResponse("No token", status_code=500)
    emps = get_adp_employees(token)
    if not emps: return func.HttpResponse("No emps", status_code=404)
    eid = req.params.get("employeeId")
    emp = next((e for e in emps if extract_employee_id(e)==eid), emps[0])
    return func.HttpResponse(json.dumps(emp, indent=2), mimetype="application/json")
...
def test_create_local_user(req: func.HttpRequest) -> func.HttpResponse:
    ...
    conn = Connection(server, user=ldap_usr, password=ldap_pwd, authentication=NTLM, auto_bind=True)
...
def process_request(req: func.HttpRequest) -> func.HttpResponse:
    token = get_adp_token()
    if not token: return func.HttpResponse("Token fail", status_code=500)
    emps = get_adp_employees(token)
    if emps is None: return func.HttpResponse("Fail emps", status_code=500)
    sorted_emps = sorted([e for e in emps if get_hire_date(e)], key=lambda e: get_hire_date(e), reverse=True)[:5]
    out=[]
    for emp in sorted_emps:
        fn = get_full_name(emp.get("person",{}))
        eid=extract_employee_id(emp)
        hd=get_hire_date(emp); td=get_termination_date(emp)
        out.append({
            "name": fn,
            "employeeId": eid,
            "hireDate": hd,
            ...
            "samAccountName": sanitize_string_for_sam((fn.split()[0][0].lower()+fn.split()[1].lower()) if fn and len(fn.split())>1 else ""),
            "email": f"{sanitize_string_for_sam(fn.split()[0].lower())}{sanitize_string_for_sam(fn.split()[1].lower())}@cfsbrands.com" if fn and len(fn.split())>1 else ""
        })
